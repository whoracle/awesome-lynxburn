...
- `widgets.lua` owns the current wibar assembly and the remaining explicitly
  theme-owned shell pieces such as wallpaper application, tasklist/taglist
  composition, and clock/date/power-menu placement
- simple in-bar non-`lx*` widgets belong in `lxbar` via
  `lxmodules.lxbar.custom_widgets`, not in the theme shell
